 ...
 *
 * Each group owns one optical lens. The lens has no content: it only refracts
 * the real controls below it through generated SVG displacement maps.
 */
type LensRect = {
    x: number;
    y: number;
...
const LENS_CLASS = 'liquid-glass-lens';
...
function createDisplacementMap(type: 'zoom' | 'bezel', size = 256): string {
        ...
        for (let x = 0; x < size; x += 1) {
            ...
            if (distance < 1) {
                if (type === 'zoom') {
                    ...
                    const rim = smoothstep(0.48, 0.82, distance)
                        * (1 - smoothstep(0.82, 1, distance));
...
function createSpecularMap(size = 256): string {
    const canvas = document.createElement('canvas');
    canvas.width = size;
    canvas.height = size;
    ...
    for (let y = 0; y < size; y += 1) {
        for (let x = 0; x < size; x += 1) {
            ...
            const directionX = distance ? nx / distance : 0;
            const directionY = distance ? ny / distance : 0;
            const rim = smoothstep(0.56, 0.84, distance)
...
class LiquidGlassGroup extends HTMLElement {
    private static nextFilterId = 0;
    private lens: HTMLSpanElement | null = null;
    private target: HTMLElement | null = null;
    private motion: LensMotion = { entrance: null, exit: null };
    ...
    private ensureFilter(): SVGFilterElement | null {
        ...
        const id = `liquid-glass-lens-${LiquidGlassGroup.nextFilterId += 1}`;
    ...
    private sizeFilter(rect: LensRect): void {
    ...
    private ensureLens(): HTMLSpanElement | null {
    ...
    private findItem(eventTarget: EventTarget | null): HTMLElement | null {
    ...
    private lensRectFor(item: HTMLElement): LensRect {
    ...
    private render(rect: LensRect): void {
    ...
    private renderBetween(pointer: Point): void {
        ...
        const radius = this.dataset.liquidVariant === 'cards'
            ? `${lerp(fromRadius, toRadius, progress) + blend * 4}px`
            : '999px';
    ...
    private nearestOtherItem(pointer: Point, source: HTMLElement): HTMLElement | null {
        const groupRect = this.getBoundingClientRect();
    ...
    private updateFromPointer(event: PointerEvent): void {
        ...
        const groupRect = this.getBoundingClientRect();
        const margin = 16;
        const inside = event.clientX >= groupRect.left - margin
            && event.clientX <= groupRect.right + margin
            && event.clientY >= groupRect.top - margin
            && event.clientY <= groupRect.bottom + margin;
        if (!inside) {
            this.scheduleHide();
            return;
        }
